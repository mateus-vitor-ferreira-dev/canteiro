## O que foi feito

<!-- Explique em poucas linhas o que este PR muda. -->

## Requisitos atendidos

<!-- Ex.: RF03, RN04. Apague se não se aplica. -->

## Como testar

<!-- Ex.: ./mvnw test, ou os passos para ver a mudança no jogo. -->

## Checklist

- [ ] O PR aponta para a `develop` (não para a `main`)
- [ ] `./mvnw verify` passa na minha máquina
- [ ] Classes e métodos públicos novos têm Javadoc (`@author`, `@param`, `@return`, `@throws`)
- [ ] Nenhum `import javax.swing` / `java.awt` dentro do modelo
- [ ] Métodos com até 40 linhas e classes com até 400
